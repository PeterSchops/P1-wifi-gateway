...
{
  Yield_Delay(200); //We're waiting to flush everything thoroughly first!
  while (client.available()) { // flush everything in the cache    
    client.read();
  }
  ...
  for (int attempt = 0; attempt < MAX_ATTEMPTS; ++attempt) {
    ...
    Yield_Delay(1000 * (attempt + 1)); // Increasing delay between attempts
  }
...
void TelnetMgr::SendDebug(String payload)
{
  ...
  // +3 for: 2 spaces and the null character '\0'
